...
// Script creates recursive hierachy calculation item for each selected hierarchy
...
// Check for exiting calculation item or create, 
    ...
    foreach(var ci in cgtHierachyCalcGroupTable.CalculationItems) {
        if(ci.Name == strHierachyName + " Recursive") {
            ciHierachy = ci;
...
// generate name of calcualtion item as hierachy name + Recursive
        ciHierachy = cgtHierachyCalcGroupTable.AddCalculationItem(strHierachyName + " Recursive");
    }
...
VAR _TooDeep = 
    MAX ( '" + h.Table.Name + @"'[Path Length] ) + 1 < " + strScope + @"
